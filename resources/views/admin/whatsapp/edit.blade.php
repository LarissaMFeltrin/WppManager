...
@section('content_header')
    <h1><i class="fab fa-whatsapp"></i> Editar Instancia WhatsApp</h1>
@stop

@section('content')
<div class="card">
    <form action="{{ route('admin.whatsapp.update', $whatsapp) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="session_name" id="session_name"
                               ...
                               value="{{ old('session_name', $whatsapp->session_name) }}" required>
                        @error('session_name')
                        ...
                        <select name="empresa_id" id="empresa_id" class="form-control select2" required>
                            ...
                            @foreach($empresas as $empresa)
                                <option value="{{ $empresa->id }}"
                                    {{ old('empresa_id', $whatsapp->empresa_id) == $empresa->id ? 'selected' : '' }}>
                                    {{ $empresa->nome }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <div class="custom-control custom-switch">
                            <input type="checkbox" name="is_active" value="1" class="custom-control-input"
                                   id="is_active" {{ $whatsapp->is_active ? 'checked' : '' }}>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="alert alert-info">
                        ...
                        Telefone conectado: <strong>{{ $whatsapp->phone_number }}</strong>
                    </div>
                </div>
            </div>
            @endif
