...
const MEDIA_DIR = process.env.MEDIA_DIR ? path.resolve(__dirname, '..', process.env.MEDIA_DIR) : path.resolve(__dirname, '../storage/media');
...
app.post('/api/forward-message', async (req, res) => {
    try {
        ...
        if (origMsg.message_type === 'text' || !origMsg.media_url) {
            ...
            // media_url no banco: /storage/media/xxx.jpg ou /uploads/media/xxx.jpg
            const mediaFileName = path.basename(origMsg.media_url);
            const mediaPath = path.join(MEDIA_DIR, mediaFileName);
